...
export function Dashboard() {
  ...
  // Helper function to render imoveis content
  const renderImoveisContent = () => {
    if (loadingImoveis) {
      return (
    ...
    if (imoveis.length === 0) {
      return (
        <div className="w-full">
          <div className="rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50/50 p-8 md:p-12 lg:p-16 text-center">
            <div className="w-24 h-24 rounded-full bg-slate-100 flex items-center justify-center mx-auto mb-6">
              <Home className="w-10 h-10 text-slate-400" />
            </div>
            <h3 className="text-2xl font-semibold text-slate-900 mb-2">Nenhum imóvel cadastrado</h3>
            <p className="text-slate-500 mb-8 max-w-2xl mx-auto">Comece criando seu primeiro imóvel para aparecer aqui.</p>
            <button
              onClick={handleNovoImovel}
              className="inline-flex items-center gap-2 px-8 py-4 rounded-xl bg-slate-900 text-white text-base font-medium hover:bg-slate-800 transition-all shadow-sm"
            >
              <Plus size={22} />
              <span>Criar primeiro imóvel</span>
            </button>
          </div>
        </div>
      );
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 transform transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 ${isDesktop ? "translate-x-0" : sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="p-3 border-t border-slate-200 lg:hidden">
          ...
          <button
            ...
            className="mt-2 w-full flex items-center justify-center gap-2 rounded-xl px-4 py-2 text-sm font-medium text-red-600 hover:bg-red-50 transition-colors"
          >
            <LogOut size={20} />
            Sair
          </button>
      ...
      <main className="flex-1 lg:ml-64 min-h-screen min-w-0">
        {/* Top Bar - Compact */}
        <header className="sticky top-0 z-30 h-14 bg-white/95 backdrop-blur-sm border-b border-slate-200 px-4 lg:px-6">
            ...
            <>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
                <div className="min-w-0">
                  <h2 className="text-xl font-bold text-slate-900 truncate">Meus Imóveis</h2>
                ...
                <button
                  onClick={handleNovoImovel}
                  className="flex items-center gap-2 px-5 py-2 rounded-xl bg-slate-900 text-white text-sm font-medium hover:bg-slate-800 transition-all duration-200 shadow-sm hover:shadow-md w-full sm:w-auto"
                >
                  <Plus size={18} />
              ...
              {/* Properties Grid - RESPONSIVE AUTO-FILL */}
              {loadingImoveis ? (
                <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  {[1, 2, 3, 4].map((i) => (
                    <div key={i} className="animate-pulse">
